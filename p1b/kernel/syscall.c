...
// NEW SYSTEM CALL SYS_getnumsyscallp added at end of table
...
syscall(void)
{
  ...
  if(num > 0 && num < NELEM(syscalls) && syscalls[num] != NULL) {
	...
	// Decrement the counter by 1 to account for getnumsyscall itself
	if(num == 22) {
		proc->counter--;
	}

	// Increment the counter for each successful system call
	// excluding our new getnumsyscallp system call
	proc->counter++;
